# Standalone runner for the Requirements Agent
...
from agents import requirements_agent
...
if "requirements_result" not in st.session_state or not st.session_state.requirements_result:
    load_project_state()
...
# ---------------------------------------------------------------
st.markdown("""
<div style="background: radial-gradient(circle at 50% 30%, #faf5ff 0%, #f3e8ff 50%, #fffbeb 100%); padding: 2rem; border-radius: 16px; border: 1px solid #e2e8f0; margin-bottom: 2rem; text-align: center;">
    <div style="font-weight: 800; font-size: 0.85rem; letter-spacing: 0.08em; text-transform: uppercase; color: #9333ea; margin-bottom: 0.5rem;">📑 IdeaToIndia Standalone Agent</div>
    <div style="font-size: 2.2rem; font-weight: 800; color: #0f172a; margin-bottom: 0.5rem; font-family: 'Playfair Display', serif;">Product Requirements (PRD)</div>
    <div style="color: #475569; font-size: 0.95rem; max-width: 600px; margin: 0 auto;">
        Define functional/non-functional requirements, target user personas, key product modules, and acceptance criteria.
    </div>
</div>
...
with st.container(border=True):
    ...
    shared_idea = st.text_area(
        "Describe your core business idea (automatically shared with the Requirements Agent)",
...
# ---------------------------------------------------------------
# Render standalone Requirements Agent
# ---------------------------------------------------------------
